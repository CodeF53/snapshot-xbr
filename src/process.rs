...
pub fn process(img: image::DynamicImage, tile: bool, relayer: bool) -> image::ImageBuffer<image::Rgba<u8>, Vec<u8>> {
...
fn scale_tiled(img: &image::DynamicImage) -> image::ImageBuffer<image::Rgba<u8>, Vec<u8>> {
    ...
    tiled = image::imageops::crop(&mut tiled, width - 4, height - 4, width + 8, height + 8).to_image();
    let tiled_width = tiled.width();
    let tiled_height = tiled.height();

    let out_rgba = xbrz::scale_rgba(tiled.as_raw(), tiled_width as usize, tiled_height as usize, 4);
